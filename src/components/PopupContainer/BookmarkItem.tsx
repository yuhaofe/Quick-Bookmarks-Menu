...
export default function BookmarkItem(props: BookmarkItemProps) {
   ...
   * @see https://stackoverflow.com/a/76622068/5819157
   */
  function faviconURL() {
  ...
  return (
    <div
      className={`bookmark-item${!props.active ? ' bookmark-item-hide' : ''} ${props.active || config.showHidden ? 'show-flex' : 'hide'}`}
      data-id={props.id}
    >
      <button
        className="bookmark-item-button"
        role="link"
        tabIndex={0}
        title={props.type === 'link' ? props.title + "\n" + props.url : ""}
        type={props.type}
        onClick={handleClick}
        onMouseOver={handleMouseOver}
        onMouseOut={handleMouseOut}
        onWheel={handleMouseOut}
        onMouseUp={handleMouseUp}
        onMouseDown={handleMouseDown}
      >
        <img
          className={`bookmark-item-icon bookmark-item-icon-${props.type}`}
          src={faviconURL()}
        />
